...
class HPSP_Admin {
	...
	public static function enqueue( $hook ) : void {
		...
		wp_enqueue_style( 'hpsp-admin', HPSP_URL . 'assets/css/admin.css', [], HPSP_VERSION );
		wp_enqueue_script( 'hpsp-admin', HPSP_URL . 'assets/js/admin.js', [ 'jquery', 'wp-color-picker' ], HPSP_VERSION, true );
	...
	public static function render_page() : void {
		...
		<div class="wrap hpsp-wrap">
			...
			<div class="hpsp-layout">
				...
				<div class="hpsp-side">
					<?php self::render_preview_box( $settings ); ?>
	...
	protected static function render_events_tab( array $settings ) : void {
		...
		<p class="description hpsp-tokens-help">
			<?php esc_html_e( 'Templates support basic HTML (links, bold, italics) and dynamic tokens. Tokens without a value for a given event are left blank.', 'social-proof-for-hivepress' ); ?>
		</p>
		<?php
		foreach ( HPSP_Events::types() as $type => $config ) :
			...
			<div class="hpsp-event-card">
				...
				<p>
					...
					<input type="text" id="hpsp-template-<?php echo esc_attr( $type ); ?>" class="large-text" name="<?php echo esc_attr( $base ); ?>[template]" value="<?php echo esc_attr( $event['template'] ); ?>">
				</p>
				...
				<p>
					...
					<select id="hpsp-image-<?php echo esc_attr( $type ); ?>" name="<?php echo esc_attr( $base ); ?>[image]">
	...
	protected static function render_design_tab( array $settings ) : void {
		...
		<table class="form-table" role="presentation">
			<?php
			self::field_select( 'position', __( 'Desktop position', 'social-proof-for-hivepress' ), '', $settings, $positions );
		...
		<table class="form-table" role="presentation">
			...
			self::field_number( 'animation_speed', __( 'Speed (ms)', 'social-proof-for-hivepress' ), '', $settings, 100, 2000, 50 );
			?>
		</table>
		...
		<table class="form-table" role="presentation">
			<?php
			self::field_color( 'bg_color', __( 'Background', 'social-proof-for-hivepress' ), $settings );
			self::field_color( 'text_color', __( 'Text', 'social-proof-for-hivepress' ), $settings );
			self::field_color( 'link_color', __( 'Links', 'social-proof-for-hivepress' ), $settings );
			self::field_color( 'border_color', __( 'Border', 'social-proof-for-hivepress' ), $settings );
			self::field_number( 'border_width', __( 'Border width (px)', 'social-proof-for-hivepress' ), '', $settings, 0, 10 );
			...
			self::field_number( 'font_size', __( 'Font size (px)', 'social-proof-for-hivepress' ), '', $settings, 10, 24 );
			self::field_number( 'max_width', __( 'Max width (px)', 'social-proof-for-hivepress' ), '', $settings, 240, 640 );
			self::field_select(
				'image_style',
			...
			self::field_checkbox( 'show_close', __( 'Show close button', 'social-proof-for-hivepress' ), '', $settings );
	...
	protected static function render_timing_tab( array $settings ) : void {
		...
		<table class="form-table" role="presentation">
			...
			self::field_number( 'gap', __( 'Gap between popups (seconds)', 'social-proof-for-hivepress' ), '', $settings, 1, 120 );
			self::field_number( 'max_visible', __( 'Popups visible at once', 'social-proof-for-hivepress' ), '', $settings, 1, 5 );
	 ...
	 * @param array $settings Current settings.
	 */
	protected static function render_preview_box( array $settings ) : void {
		$user_id = get_current_user_id();
	...
	protected static function field_number( string $key, string $label, string $description, array $settings, int $min, int $max, int $step = 1 ) : void {
		...
		<tr>
			...
			<td>
				<input type="number" id="hpsp-<?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( self::name( $key ) ); ?>" value="<?php echo esc_attr( $settings[ $key ] ); ?>" min="<?php echo esc_attr( $min ); ?>" max="<?php echo esc_attr( $max ); ?>" step="<?php echo esc_attr( $step ); ?>" class="small-text" data-hpsp="<?php echo esc_attr( $key ); ?>">
				<?php if ( $description ) : ?>
					<p class="description"><?php echo esc_html( $description ); ?></p>
				<?php endif; ?>
			</td>
	 ...
	 * Colour picker row.
	 */
	protected static function field_color( string $key, string $label, array $settings ) : void {
		?>
		<tr>
			<th scope="row"><label for="hpsp-<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $label ); ?></label></th>
			<td>
				<input type="text" id="hpsp-<?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( self::name( $key ) ); ?>" value="<?php echo esc_attr( $settings[ $key ] ); ?>" class="hpsp-color" data-hpsp="<?php echo esc_attr( $key ); ?>">
			</td>
		</tr>
		<?php
	}
